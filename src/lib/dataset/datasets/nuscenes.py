...
class nuScenes(GenericDataset):
  ...
  def run_eval(self, results, save_dir):
    ...
    if task == 'det':
      ...
      print('evalutaing '+'{}/results_nuscenes_{}.json '.format(save_dir, task)+'......')
      os.system('python ' + \
                'tools/nuscenes-devkit/eval/tracking/evaluate.py ' + \
                '{}/results_nuscenes_{}.json '.format(save_dir, task) + \
                '--output_dir {}/nuscenes_evaltracl_output/ '.format(save_dir) + \
                '--dataroot ../data/nuscenes/v1.0-trainval/')
